...
删除任务只移除被点名的那一条：它的直接子任务解除父子引用并成为独立任务，各自的子树、负责人、评论、附件与历史全部保留，每个被提升的子任务会记录一条层级变更活动。因此删除不再要求节点数确认，也不再需要调用者能删除子树中的每一个节点；被删除任务自身的附件进入既有软删/硬删清理流程，来源行动项解除任务引用，删除通知保存必要快照后可继续重试。移动父任务仍然原样移动整棵子树，并保留整树可见性与 `confirm_subtree_node_count` 确认。
...
子任务不会继承或扩大父任务的**写入**权限：修改、删除、评论、附件都仍按任务自身的创建人、负责人、关注人和清单角色判定。可见性则向上闭合——能读到一条任务，就能**只读**读到它上面的完整父链，包括父任务的详情、评论和附件元数据；兄弟任务不在这条链上，因此不会一并可见。会话分享只作用于明确分享的任务，不向上传播，分享前仍要求整条父链已分享进同一会话。部署可通过 `TASK_UPWARD_VISIBILITY=false` 关闭向上可见，回到"父链不可见即整条链不可见"的严格行为。
