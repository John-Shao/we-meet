...
状态:阶段一 + 阶段二**已上线**(2026-07-15;双栏页 + 扫码建会话 + SSO 全链路实测通过,**realm 全局 `browserFlow=unified-browser` 已铺开**,meet/docs 均走双栏、互通免登实测通过,meet client 冗余 override 已清空跟随 realm。`scan-browser` 已退役,`phone-browser` 暂留作回滚保险(unified 稳定后用 `retire-legacy-flows.sh` 退)。踩坑:改 theme 后 login.css 按 KC 版本号缓存,须无痕/硬刷才见新样式)
