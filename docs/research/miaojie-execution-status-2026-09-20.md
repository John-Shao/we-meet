...
| 6 | 分片合并响应丢失及登记失败恢复，双端免重传，进度累计 | 见本文件所在批次提交 | 见 Android 同批提交 |
...
本轮关闭了优先级最高的正确性及回看链路缺口。复评中的共享范围与原媒体下载、发言占比/时间轴、独立章节导航及空态、批量校对、长期热词库、关键词、多条件全局筛选排序仍属于后续产品批次；线上会议完整录像回放另需实际录制产物和媒体时间映射。不要把这六批理解为已全量对齐飞书妙记。
