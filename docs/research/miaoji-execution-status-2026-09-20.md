...
| 30 | 联合生产验收预检：新接口 404，等待部署；未完成验收 | 本批文档 | 无运行变更 |
...
[第三十批预检](miaoji-batch-30-deployment-preflight.md)：第 28/29 批完成并推送后自动进入联合验收。线上个人热词路由返回 404，现有记录路由返回 401，新接口尚不可用，生产验收停在部署前置条件；未登录或写入生产测试内容。需要按现有流程发布 backend/frontend（含 0183/0184），再继续第 26–29 批验收。当前运行代码为 Web/后端 `95a06f25d`、Android `a2a9a5b0`。
...
最新修正：第 28 批已实现单时钟片段级发言分布，第 29 批已实现更多个人导出历史；下文早期阶段中对应“仍缺少”表述仅保留历史。G6 仍缺可靠媒体时长与人工历史版本精确导航，G8 仍缺多时钟映射、超预算渐进加载和词级时间。组合筛选、记录级回收站等仍未实施；这些开发缺口与第 30 批的生产部署阻塞分别记录。
