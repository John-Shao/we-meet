...
修复提交 `8cada0f4f` 已推送，对应 production agents 镜像已构建并推送到 `jusi-cn-guangzhou.cr.volces.com/we-meet/meet-agents:8cada0f4f`，仓库 digest 为 `sha256:b3ebe0a848054c0901dc3531d8f1a6d4c3607dfa439b6af739825e73d55a58d9`。Linux 镜像在禁网环境完成模块导入和 OSS 签名 smoke 检查。与生产 `f2e7f3cd4` 相比，agents 运行源码仅本批适配器改动。

生产主机下一步：

```bash
bash deploy/aliyun/release-meet.sh --tag 8cada0f4f agents
```

固定 `--tag`，不要将随后验收文档提交的 SHA 当作已构建镜像。生产发布及原样本新代次转写仍待完成。
