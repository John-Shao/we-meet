...
7. 会议首页的「录音」：开始、暂停、刷新、继续、断网重试和结束；关闭采集开关后仍可结束已有录音。检查真实对象存储权限、WAV 字节、缺片清单以及账号切换后的本地隔离。开启独立 ASR 并部署对应 Worker 后，保存录音可手动转写、取消、重新转写及按文字时间点回听；缺片须明确接受，网络失败后确认同一次请求。独立智能纪要仍待连接。
