...
用户负责部署和实测，本文件记录开发侧现状。新开关默认关闭，尚未执行生产迁移或修改线上配置。累计第二十八批是阶段性增量，不是完整首版交付。
...
7. 会议首页的「录音」：开始、暂停、刷新、继续、断网重试和结束；关闭采集开关后仍可结束已有录音。检查真实对象存储权限、WAV 字节、缺片清单以及账号切换后的本地隔离。本轮独立录音只保存音频。

## 迁移与运行组件

备份并按原部署流程执行 `python manage.py migrate --noinput`。本轮最新迁移为 `0164_capture_audio_interruption`；先在测试库演练现有数据到最新版本。开发侧隔离数据库已执行通过。
...
- 独立录音已连接 Web 麦克风、本地日志、真实 WAV 分片保存、回执恢复和受保护回放；独立 ASR 与仅文字清理继续实现，真实后端端到端测试由部署后验证。当前按分片播放，切换时可能短暂缓冲。
