...
日期：2026-09-21 · 更新：2026-09-24 · 修订：v1.14，沟通准备模型启用与预检 · 状态：材料四种格式已通过生产验收；9 月 24 日服务器确认材料开启、消费者 1，沟通关闭且模型未配置；新增分步启用 / Worker 模型探针待部署，真实业务与桌面办公验收待完成；D0 剩余发布验收独立跟踪 · 原源码核查基线：we-meet `4c5ca713f`；办公第二批实施基线：`09a7841f9`
...
本节是当前执行顺序的唯一依据。v1.9 将 P0-0 至 P0-3 重排为材料底座、沟通准备、周报、表格分析；原 P0 中的会议 / 待办 / 云文档集成移入独立 P1-M。v1.10 已完成材料子集的本地验收，其他批次仍待实施；D0 保留在独立支线，不以本文完成或 PoC 演示作为完整批次的完成依据。
...
当前优先级：将 Work 独立配置显式绑定到项目已接入的 `qwen3.8-flash`，完成沟通准备真实效果 / 延迟 / 用量验收，再进入周报，之后表格分析。核查确认会议 backend / summary 的部署配置已使用该模型、现有兼容地址及 `meet-ai-credentials/DASHSCOPE_API_KEY`，并有真实总结调用通过记录；无需重新申请模型凭证，撤回新增 Qwen3.7-Plus 首选的建议。Work 最新回执仍为 `model_configured=false`，故已有会议接入与办公配置绑定必须区分。配置示例已提供复用方式，未修改生产 overlay 或开启沟通生成。表格数值继续交给确定性代码计算，模型负责解释。接入证据、价格依据与配置集中见 [Work 工程说明](../../src/backend/work/README.md)。
