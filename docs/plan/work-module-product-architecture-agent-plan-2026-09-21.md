...
日期：2026-09-21 · 更新：2026-09-23 · 修订：v1.10，落地首批私人材料上传与解析预览 · 状态：P0-0 材料子集已通过本地验收，办公生成与生产部署待完成；D0 剩余发布验收独立跟踪 · 原源码核查基线：we-meet `4c5ca713f`；办公首批实施基线：`961147f50`
...
| 模型 | T2 前核实已有 Qwen 账号；T5 前通过真实结构化输出、引用及用量验证。新增 `WORK_MODEL / WORK_BASE_URL / WORK_ENABLED` 等独立设置，通过 LLMClient 显式参数构造，不直接绑定会议 `from_settings()` | 无模型时继续上传、预览和契约测试；P0-1 不放行。费用未知标记未定价，不当成免费；不自行变更会议配置或替换未核实端点 |
...
P0-0 仍保持未整体勾选：WorkTask / WorkRun / Artifact 的通用执行账本、模型预检及生产私有存储尚未完成。下一步在材料底座上增加目标、选定版本、持久运行和沟通准备草稿，补 PDF / DOCX 后验收 P0-1；周报和表格按原顺序推进。任务级配额随任务模型补齐，当前材料空间已有独立账号配额。
