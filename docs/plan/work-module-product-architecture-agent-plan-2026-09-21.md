...
日期：2026-09-21 · 更新：2026-09-23 · 修订：v1.12，补齐 Work 独立 Worker、部署检查与材料启用入口 · 状态：P0-1 代码已随 `e82d71f91` 部署，Work 开关尚未启用；真实模型 / 生产业务 / 桌面办公验收待完成；D0 剩余发布验收独立跟踪 · 原源码核查基线：we-meet `4c5ca713f`；办公第二批实施基线：`09a7841f9`
...
**P0-0 / P0-1 仍不整体勾选**：真实模型配置和合成样本效果 / 用量验收尚未完成；生产私有存储、工作池资源 / 网络边界和桌面办公冒烟仍待验证，没有部署 `meet.we-meet.online` 或更新已安装桌面包。下一步先完成这些放行项，再进入 P0-2 周报；现有实现可复用材料选择、执行账本、引用、版本与下载能力，会议主线继续独立迭代。
...
当前下一步：在发布主机拉取代码并执行材料启用脚本，成功后用 demo 账号验证生产上传 / 解析 / 删除；补上独立 Work 模型 Secret 引用和真实生成验收，再进入周报。工作机尚无生产集群 / SSH 连接，不能把本地脚本测试当成已在京东云启用。执行命令、关闭方式及配置位置统一见 [Work 工程说明](../../src/backend/work/README.md)，不新增零散计划文件。
