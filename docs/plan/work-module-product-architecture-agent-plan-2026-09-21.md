...
日期：2026-09-21 · 更新：2026-09-22 · 修订：v1.4，依据 WorkBuddy 5.5.6 中文版 8 张截图补齐三场景、助理、项目模板、能力中心、定时任务与资料库；保留 v1.3 执行内核选型结论 · 状态：规划建议，尚未实施 · 原源码核查基线：we-meet `4c5ca713f`
...
首期以“会议与协作闭环”为切入点，同时具备上传文件和生成办公成果的基础能力；随后增加本地文件执行、手机续办、项目与资料管理，以及代码开发和设计创意试点，最后扩展桌面 GUI、第三方连接器和团队专家。会议是首发验证场景，Work 的产品边界覆盖通用办公、开发、创意与自动化；代码开发、设计创意和开放生态扩展单独估算资源，不默认为原 P0 排期已包含。

建议技术组合：**现有 React / Django + LangGraph Python + PostgreSQL 检查点 + 独立 Celery 队列 + 工具权限网关 + 云端沙箱 / 桌面执行器 + 可替换的模型适配层**。首期使用单个主 Agent 配合受控工具；专业能力先实现为技能，不默认启动多个 Agent。

上述为初版架构基线，尚未定版。v1.3 建议：**自有产品内嵌执行内核优先验证 Pi，桌面通用助手路线对比 Goose；DeepSeek Harness（dsh）保留为候选，OpenHands SDK 留作代码开发专项执行器候选**。Django 持有业务状态，执行运行时经 adapter 接入；LangGraph 按跨业务流程需要选择性保留，不叠加管理同一个工具循环。此为文档核查后的工程判断，尚无实测排名；最新选择规则见第 19 节，第 18 节保留 dsh 专项分析。

这是一项面向现有系统的产品与工程建议，不代表竞品内部架构，也不代表已验证的模型性能。默认假设：国内企业协作场景优先、复用现有组织与身份体系、先推出受控试点；部署区域、可投入人力及具体模型账号尚未核实。本文未调用付费模型、未修改业务代码、未部署服务。
...
以下沿用原代码基线的源码核查，本次截图修订未重新审计业务实现；不把 README 的历史规划或默认配置等同于线上可用。
...
| 桌面应用 | [main.ts](../../src/desktop/src/main.ts)、[preload.ts](../../src/desktop/src/preload.ts) | 当前仅加载 React、隔离 renderer 并暴露平台信息；设备代理、文件授权、配对、自动更新等需要新增或完成 |
...
| P1 | 整理我电脑上的这些文件 | 文件清单、变更预览、处理后的副本 | 限定目录；冲突可处理；覆盖前可确认；源文件可恢复 |
...
| WorkWorkspace / WorkspaceMount | P1：文件根引用、仓库、基线版本、环境挂载、读写 grant；切换设备不自动继承挂载授权 |
...
| 执行环境 | 云端隔离沙箱；P1 桌面受控执行器 | 数据、代码与业务服务分离，统一任务模型 |
...
先建立 100 条固定任务，建议组成：会议与消息 30、资料写作 20、表格分析 20、文件处理 10、权限 / 注入 / 故障恢复 20。P0 文件处理使用上传文件或沙箱，不以尚未完成的桌面代理作为必需条件；P1 追加配对、离线、撤销和跨端冲突样本。
...
估算基于：1 位产品 / 设计、2 位后端 / Agent 工程师、1 位 Web / Electron 工程师、1 位测试，Android 与基础设施有人配合。估算未经排期确认，单人开发不能直接沿用此日历周期。

| 阶段 | 建议周期 | 交付范围 | 放行条件 |
|---|---|---|---|
| 验证阶段 | 第 1–2 周 | 三场景及管理页原型、100 条评测集初版、Qwen / Doubao 对比、按第 19 节收敛内核 PoC、权限工具样例 | 读取会议→生成草稿→确认→创建一个待办全链路；模拟崩溃不重复写入 |
| P0 协作与云端成果 | 第 3–8 周 | Work 首页 / 详情 / 成果，四个首期场景，运行持久化、受控沙箱、确认、通知、预算 | 受测权限无越界；验收集与采纳反馈过线；可灰度关闭 |
| P1 本地与跨端 | 原估 P0 后约 4–6 周；本次管理页增量须拆包重估 | Windows 设备代理、目录授权、助理与手机续办；项目模板、定时规则 / 记录、资料库、预设专家与自有连接管理；PPT 按容量安排 | 配对、撤销、离线、文件冲突、恢复与 UI-05–11；桌面签名与更新通道准备完成 |
| P1-C 代码开发试点 | 另估 2–4 周；需至少 1 位熟悉开发工具的工程师及测试配合 | 隔离仓库、文件修改、受控测试、diff / patch、开发成果预览 | 仓库状态保护、命令隔离、取消恢复与成果验证；可在资源充足时与 P1 并行 |
| P1-D 设计创意试点 | 单独估算；图像服务 / 模板 PoC 后确定，不挤入原 P1 周期 | 视觉 / 运营海报、图片生成 / 编辑、风格与尺寸参数；复用已有 PPT 能力；输出版本与素材引用 | UI-04、图片 / PPT 可用性、中文文字与布局、引用素材范围、异步失败与费用核对；依赖稳定的成果服务 |
| P2 团队与扩展 | P1 后原估约 6–10 周，按需求拆包 | GUI / 云桌面、外部连接器、团队专家 / 专家团、有限多 Agent、企业技能治理；视频、完整品牌资产包与 macOS 单列验证 | 指定应用成功率可接受；组织级审计、撤权、成本与隔离验证 |

P0 补充 Ask / Plan / Execute 的服务端模式约束与验收；代码开发入口在 P1-C 达标后开放，设计创意入口在 P1-D 至少一个完整场景达标后开放。复用 PPT 技能时办公与创意可以指向同一技能和成果流水线。行业 WorkApp 与硬件伙伴生态在 P2 后按合作项目单独立项，不包含在上述时间估算中。

P0 自然语言调度、云端产物和业务闭环并行推进；桌面 GUI 不阻塞 P0。桌面打包签名、登录回跳、更新渠道是 P1 前置；Android 第一阶段可以先通过响应式 Work 页面验证，原生入口、推送和深链需要独立任务与验收。
...
| W09 项目及模板 | P1；依赖 W02、W05 | 列表 / 搜索、成员与规则、版本化模板创建、项目任务树 |
| W10 助理及渠道状态 | P1；依赖 W02、设备与第 17.1 节渠道契约 | 会话与任务关联、设备 / 渠道分开显示、续办和指代澄清 |
| W11 能力中心 | P0 技能清单，P1 管理，P2 发布；依赖工具注册和权限网关 | 专家 / 技能 / 连接器页签、依赖详情、选用、授权失效 / 撤销状态 |
| W12 定时管理 | P1；依赖执行恢复、授权和通知链路 | 规则编辑 / 批量操作、触发槽去重、运行记录与详情跳转 |
| W13 资料库 | P1；依赖 W06、文档 API 与 W09 项目权限 | 来源引用索引、个人 / 团队与分享视图、本地产物、成果再引用 |
| W14 创意成果 | P1-D；依赖 W06、W08 和图像服务验证 | 海报 / 图片 / PPT 子场景、结构化风格参数、预览与版本、质量评测 |

依赖顺序为“公共任务 / 权限 / 成果契约 → 各管理页最小闭环 → 代码与创意专项 → 团队发布和外部生态”。W09–W14 的前后端、设计和测试增量需重新核算，不能只把导航做出来就视为功能交付，也不能把这些工作全部塞入原 4–6 周估算。

建议目录增量：`src/backend/work/{models,api,services,runtime,tools,tests}`；`src/frontend/src/features/work/{routes,components,api}`；P1 `src/desktop/src/work` 与 `src/work-runner`。新增 app 需注册与迁移，不继续把所有 Work 模型堆进已有大型 `core/models.py`。
...
1. 首发主场景：默认会议与协作闭环；若本地文件优先，将桌面设备代理提前，但不压缩权限、操作账本和恢复工作。
2. 部署：单组织私有部署还是多组织 SaaS；模型、检索与文件执行的数据允许范围。
3. 客户端：Windows 是否首发，macOS 是否同期；Android 原生投入与桌面签名 / 更新渠道。
4. 模型与配额：可用账号、具体版本、并发额度和月预算；本文没有账号实测或价格报价。
5. 云文档 API：一般文档读取 / 创建 / 更新与来源权限映射是否满足需求；当前确认的是会议导出已有可靠交付模式。
6. 管理能力优先级：项目模板、助理、定时管理与资料库按哪一客户流程先打通；新增工作包需分配设计、前后端及测试容量。
7. 创意试点范围：默认图片 / 海报优先；图像服务授权、参考素材管理、中文渲染质量与预算验证后确定 P1-D 周期，视频和完整品牌设计保持 P2 候选。

建议以“用户提供一场会议和一张表，Work 输出带引用的报告，经确认创建待办并保存文档，手机能看到进度和成果”作为首个完整演示。该闭环可同时验证产品价值、工具契约、成果质量和跨端状态，为后续本地操作与通用办公扩展提供基础。
