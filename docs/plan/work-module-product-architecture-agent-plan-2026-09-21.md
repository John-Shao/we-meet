...
当前优先级：配置独立 Work 模型并完成沟通准备真实效果 / 延迟 / 用量验收，再进入周报，之后表格分析。按用户提出的阿里云千问方向，建议先验收固定快照 `qwen3.7-plus-2026-05-26`（非思考模式），作为沟通准备和后续周报的候选；这是选型建议，尚未完成账号授权或调用。表格数值继续交给确定性代码计算，模型负责解释。价格依据与模型配置集中见 [Work 工程说明](../../src/backend/work/README.md)。
