...
所有具有所属列表的独立详情页，页头统一采用 **“列表页名称 > 详情页名称”**，将返回入口与当前页面标题放在同一行。新建及修改此类页面时必须遵循此规范。

```text
列表页名称 > 详情页名称                       [次要操作] [主要操作]
             来源 · 时间等辅助信息（按需）

会议实录 > 课堂教学                           [协作者管理] [分享]
```

- **列表页名称**：使用与目标列表页面一致的名称，作为可点击链接返回所属列表；使用 `text.secondary` 和共享链接的 hover、focus-visible 状态。返回时恢复原列表的筛选、搜索、排序、分页和滚动位置；从分享链接等入口直接打开详情时，返回所属列表的默认状态，不能仅依赖浏览器历史后退。
- **详情页名称**：使用当前对象的实际名称（如“课堂教学”），作为页面唯一的主标题，采用 `headlineSmall` 与 `text.primary`，不可作为导航链接。不要再重复显示“会议实录详情”等通用标题；需要重命名时，在标题旁使用带明确标签的共享 `IconButton`。
- **层级分隔符**：规范以 `>` 表示层级，界面统一使用向右的 chevron 图标，采用次要图标色；分隔符不可点击，对辅助技术隐藏。
- **操作区**：位于页头右侧，与导航标题行垂直居中；次要操作在左、主要操作在右，同一操作组最多一个主按钮。按钮使用共享组件及尺寸契约，不插入列表名称与详情名称之间。
- **辅助信息**：来源、时间等放在第二行，与详情名称左侧对齐，使用 `bodySmall` 与 `text.secondary`；无辅助信息时不预留空行。
- **间距与适配**：使用现有 Typography、Spacing 和 Color token。桌面页头默认左右 `xl`、上下 `lg`，导航项及操作之间使用 `sm` 间距；窄屏可采用 `lg` 水平边距，并将操作区移至下一行。长标题单行省略，同时提供鼠标悬停及键盘聚焦可查看完整名称的入口；保留列表返回入口和主要操作，不能造成页面横向溢出。
- **可访问性**：层级导航使用带可访问名称的 `nav`，当前项标记 `aria-current="page"`，详情名称保留 `h1` 语义；列表链接可通过键盘访问，并有明确的焦点状态。
- **适用边界**：该规则用于具有列表—详情层级的完整页面，包括会议实录等业务模块；不为无所属列表的页面虚构上级。弹窗、抽屉沿用对应容器的标题栏规范。使用本模式的完整页面不再叠加独立的“返回列表”行或重复返回箭头。

验收时检查：名称与列表一致、返回状态恢复、直接访问的返回目标、长标题、窄屏、200% 文字缩放、键盘导航，以及明暗主题。存量详情页按页面修改逐步迁移；本规范的登记不表示已有页面已完成改造。

### 会议实录子页面工具条

- 子页面采用“页签 → 工具条 → 内容区”的结构；工具条固定在所属面板顶部，正文独立滚动。
- 分享、协作者管理、重命名及移入回收站属于整条实录，放在页头；移入回收站收进更多菜单并保留二次确认。
- 搜索、筛选、语言选择、生成、导出和下载等子页面操作集中到工具条；无操作时不保留空工具条。
- Web 工具条操作沿用文字记录样式：共享 Button 使用 `size="sm"`、`variant="secondaryText"`，搭配 16px 图标；导航链接使用同款 LinkButton，保留链接语义。悬停、焦点、禁用及加载状态沿用共享控件。
- 段落编辑、片段播放、发言人归属及分页等与具体内容有关的操作保留在内容旁。
- 文字记录、译文及译文归档使用连续滚动加载，后续批次追加并去重，不替换已读内容；底部显示加载、重试或已全部加载状态，保留“加载更多”作为键盘和辅助功能兜底。普通业务列表仍按自身需求分页。
- 刷新放在所属页签顶部工具条，保留搜索条件、已加载深度和阅读位置，不重建播放器；文字编辑时禁用刷新与自动追加。搜索覆盖完整原文，播放定位保留直接定位能力，并提供“从头阅读”入口。
- 长列表按需绘制：Android 使用 LazyColumn，Web 延迟不可见片段的布局与绘制。私有正文不写入持久化存储；来源或权限失效时清除可见正文。
- Android 常用操作直接显示，次要操作放入更多菜单；语言选择等复杂设置使用底部面板。保持至少 48dp 热区。
- 窄屏工具条不横向溢出；展开区限制高度，给正文保留阅读空间。切换页签保留浏览条件和位置，不保留隐藏页签的私有正文或后台请求。
