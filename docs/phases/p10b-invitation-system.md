...
**状态**：✅ 已交付（2026-08-02，方案 C）。commit `885ac138`（后端）+ `6521322c`（前端）。**只 commit 未 push，未上生产。**
...
**✅ 2026-08-02 拍板：方案 C。** 下文按 C 实施。开关关闭之后的 C 端半开状态（B 的那部分）**明确不在本期范围**——真有客户要关的时候另开设计；别让开关的存在造成「已经支持真准入」的错觉。
