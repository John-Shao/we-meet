...
**`/org` tab1「成员」** — 左 `Tree` 部门树 + 「仅显示直属成员」`Switch`；右 `Table`（勾选 / 姓名 / 账号状态 / 手机号掩码含行内 reveal（复用 `reveal-phone` 端点及其 `phone-viewed` 通知）/ 部门 / 人员类型 / 直属上级 / 操作）；顶栏搜索 + 状态筛选 + 人员类型筛选 + `[添加成员][批量导入][导出]`（M2-g 落地：「添加成员」按**手机号**定向录入。**未加入的人留在独立的「待接受邀请」tab，不内联进成员列表** —— 飞书同样是独立一页（「申请列表」）；混进成员列表会让服务端的部门/状态筛选筛出不受同一套条件约束的行，而 tab 上的角标已经把「有几个人待处理」说清楚了。飞书那种**邀请码/邀请链接/二维码 + 申请列表 + 审批**是另一套机制，归 M4，不在这里）；勾选后浮批量条（批量变更部门 / 批量加入用户组 / 批量离职）；行操作开 `SideSheet` 三段 `Form.Section`，对齐飞书「添加成员」弹窗：
...
- **A-Z / 拼音索引**：Web 右侧悬浮条 + sticky header；Android `AlphabetRail` + `LazyColumn.stickyHeader`
  —— **排序与字母头已交付（2026-09）**：Web 见 `src/frontend/src/features/contacts/`（悬浮字母头，仅简体中文界面）
  与 `core/services/pinyin.py`（拼音排序键，迁移 `0143`/`0144`）；Android 见 `we-meet-android` 的
