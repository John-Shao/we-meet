...
> attendee+room member,不在列表的删行并移出 room,组织者恒保留);变更卡片协议加
> 可选 ``removed_count``(老客户端忽略,badge 文案通用不受影响)。双端编辑表单对
> **非重复日程**预填既有参与者并放开增删;重复日程编辑不展示不传(服务端三选路径
> 本就剔除该字段)。后端测试 3 例新增,calendar 相关 22 过。
> web 日程详情补齐(对齐 App,`6008c98e`):详情弹窗新增参与人列表(RSVP 状态图标
> ✓/✗/? 同 App 映射 + 组织者标签,超长区内滚动)。
> 视频会议页详情面板(对标飞书,web `6f5e5e17`):预约/历史会议列表点行 → 右侧
> ResizablePanel 详情面板(标题/时间/会议号+复制/链接+复制/进入会议/查看纪要
> [历史]/删除),行内按钮与 ⋮ 菜单移除、行改纯选中;历史行不再直跳纪要页。
> App 端对齐(android `d679b6a`):预约会议行 → 新预约详情页(ScheduledDetailScreen,
> 列表数据经路由直传零请求,进会/复制会议号与链接/删除收于此);历史详情页补顶部
> 「进入会议」+ 右上删除(确认后服务端删房+清本机记录);两列表长按删除移除
> (DeletableRow 删),行为纯点击进详情;进行中的历史行保持直接重进会议。
> 实测修正(`fd7be621`):编辑态忙闲条误报——原参与者必然在该日程自身时段「忙」;
> freebusy 加可选 ``exclude_event_id`` 剔除正在编辑的日程,编辑对话框传之。顺带修
...
> P1-8 权限与参与人能力闭环(2026-08-12):Web/Android 新建与编辑表单开放
> 私密日程和必选/可选参与人；写接口新增结构化字段
> ``attendee_entries=[{user_id, role}]``，旧 ``attendee_ids`` 继续可用。
> 私密日程的非参与者凭 event id 读取时只得到时间窗和 ``details_redacted=true``，
> 标题、描述、参与人、组织者、会议室与入会信息由服务端清空；来源会话创建、
> 变更、取消卡及提醒同样不泄露标题和人员信息，真实参与人的日程助手私聊保留
> 完整详情。组织者的 RSVP 固定为接受，接口拒绝组织者自行改为拒绝/待定。
> 外部参与人已经从“日历直接填邮箱”纠偏为“通讯录先建立双向真实账号关系，再进入
> 日历、IM 和会议协作”；现状、协议、安全边界与后续二维码/组织治理蓝图统一见
> [P1b 外部联系人](./p1b-external-contacts.md)。
...
提前量**没有任何可观测效果**:界面收下了这个设置,然后什么都不做。
