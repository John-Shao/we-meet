...
- **P7-c 消息撤回**(已实现):墓碑协议消息 `content_type='recall'`、body=`{target_mid}`;前端聚合撤回集合 → 原消息渲染成「已撤回」占位、墓碑本身过滤不显示;自己 2 分钟内的消息 hover 出「撤回」按钮(确认弹窗)。不动后端,**无服务端强制校验**(任意客户端理论上能构造撤回他人消息的帧;内部工具可接受)。
