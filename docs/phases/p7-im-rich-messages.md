...
- **P7-b 文件消息**(已实现):输入区回形针按钮选任意文件 → 后端 `files/upload-url`(任意 mime、≤50 MiB)→ 发 `content_type='file'`、body=`{key,name,size}`;MessageItem 渲染文件卡片(图标+名+大小,点击下载)。**独立桶 `we-chat-doc`**,key 用 `file/` 前缀;resolve 端点按前缀路由(`chat/`→图片桶、`file/`→文件桶),图片/文件共用同一个 `images/resolve`。
