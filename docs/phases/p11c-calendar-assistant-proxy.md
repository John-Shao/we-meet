...
日程助手只解决一个明确的协作问题：日程仍指向原来源会话，但组织者已经退出，
无法再以成员身份发布后续提醒、修改或取消通知。此时由日程助手向原会话代理
发送；`event-card` 继续用既有 `organizer_name` 标明真实组织者。
...
5. 变更/取消通知为 best-effort，不影响日程操作；提醒遇瞬时错误保留未处理状态重试。

“在消息列表提醒日程”是每个用户自己的实时聚合视图，可由用户关闭，不创建
聊天消息。无来源日程只走这个入口；日程助手仅在有来源日程的组织者退出后补位
发送到期提醒，不负责客户端倒计时、每日摘要或系统自动改期。
...
`calendar_im_notify.post_with_organizer_fallback()` 为提醒和卡片提供同一发送策略：
...
- we-meet：留群提醒/修改由组织者发送；退群后的提醒、改期、参与人变化和取消
  均只产生一条日程助手消息；卡片保留组织者姓名；UID 解析失败走 SYSTEM；其他
  jusi 错误不误判；助手失败最终走 SYSTEM。
