...
| App 端时间轴 | — | 复用 `views/TimeGrid.kt` 的 `TimelineScaffold`(N 列资源轴 / 共享 hScroll / 红线 / `disabledColumn` 全现成),≈2 人日。**方向差异是刻意的**:Web 横向时间 × 纵向房间,App 纵向时间 × 横向房间(与既有日/周视图统一),别为对齐 Web 掰成横向 |
