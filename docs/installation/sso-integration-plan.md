...
5. **dead code 清理（🔸 部分完成 2026-07-15）**：已删 web 端由 KC phone-authenticator 取代的自绘手机号 OTP 组件 —— `home/components/LoginDialog.tsx`（扫码/手机号双栏容器）、`auth/components/PhoneLoginDialog.tsx`、`auth/components/PhoneLoginPanel.tsx`（commit `878ead1d`）。**刻意保留、非死代码**（见 6）：`auth/components/QrLoginPanel.tsx` + `auth/api/qrLogin.ts` + `qrcode.react` 依赖（扫码 SSO 要复用）；`auth/api/mobileOtp.ts`（`refreshTokens` 是 `fetchApi` 的 live 依赖；`sendOtp`/`verifyOtp` 现无 web 调用者，但作 mobile 契约保留）。
6. **扫码登录 SSO（未做，阻塞点已厘清）**：web 出示二维码、已登录 App 扫码授权 web 登录。前后端全套仍在（web `QrLoginPanel`/`qrLogin.ts` + 后端 `core/api/qr_login.py`），只是 web 入口被 OIDC 重定向旁路。**阻塞点在 web 侧、不在 App 侧**：现有 confirm 走 **Token Exchange 把 token 塞进 web localStorage、不在 Keycloak 域给浏览器种会话 cookie** → 扫码只登进 meet 单点，docs / 子系统仍要重登。KC 会话 cookie 只能由「web 浏览器**亲自**与 KC 完成一次认证交互」产生，App（另一设备）持任何 token 都替代不了 → **凡「异设备确认 + token 回传」模式（Token Exchange / Device Flow / CIBA）皆给不了 web 的 SSO cookie**；**App 改走 KC OIDC 也不解此阻塞**（那只换了 App 自己的凭证）。解法：把扫码做成 **Keycloak 认证流里的 authenticator**（与 phone-authenticator 并列）—— web 走 OIDC 进 KC 页、KC 页出示二维码、App 扫码确认作为「完成本次 KC 认证」的信号、由 KC 给浏览器 `Set-Cookie` + 发 code 回 meet。届时复用 `QrLoginPanel` 轮询/态机（搬进 KC theme）+ 后端 `qr_login.py` 编排，终点从 Token Exchange 换成「通知认证器放行」。「App 迁 KC OIDC」是可并行的独立收益（体系收敛），非本项前置。**完整设计见 [qr_login_sso.md](../features/qr_login_sso.md)。**
