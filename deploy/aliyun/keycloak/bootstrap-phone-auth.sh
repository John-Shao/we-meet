...
# 合成 email 域名：手机用户建号写 <手机号>@<域名> + emailVerified，
# 免 Keycloak VERIFY_PROFILE 卡手机用户手填 email/姓名。
...
CFG=$(jq -n --arg u "$GATEWAY_URL" --arg t "$GATEWAY_TOKEN" \
  ...
  '{alias:"phone-otp-config",config:{sms_gateway_url:$u,sms_gateway_token:$t,otp_length:$l,otp_expiry_seconds:$e,otp_max_attempts:$m,demo_phones:$dp,demo_otp:$dotp,email_domain:$edom}}')
if [[ -n "$CFG_ID" ]]; then
