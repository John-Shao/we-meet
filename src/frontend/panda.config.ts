...
const config: Config = {
  ...
  theme: {
    ...
    tokens: defineTokens({
       ...
       * 字号刻度,键名即像素值。
       *
       * 按实际用量重定过:原刻度缺了 11 / 13 / 15 / 17 / 18 五档,而它们恰恰是
       * 高频档位 —— 13px 用了 78 处、11px 31 处、15px 19 处、17px 6 处(全是
       * 对话框标题)、18px 13 处。刻度对不上现实,是全站 494 处字号一律写字面量、
       * 这组 token 采纳率为 0 的直接原因。顺带修了 40 档:原值 2.375rem 实为 38px,
       * 与键名不符(改动无风险,本来就没人用)。
       *
       * 行高配对(**尚未落地**,新代码可照此写):
       *   11/16  12/16  13/18  14/20  15/22  16/22  17/24  18/24
       *   20/28  24/32  28/40  32/44
       * 存量 494 处字号里只有 73 处显式给了 lineHeight,其余继承浏览器默认
       * (约 1.2),比上表更紧。回填是**真实的视觉变化**(整站文本块变高),
       * 要单独拍板,别顺手做。
       */
      fontSizes: {
        10: { value: '0.625rem' },
    ...
    semanticTokens: defineSemanticTokens({
      colors: {
        ...
        // 二级导航栏底色:各模块的「第二栏」(消息会话列表 / 通讯录 / 日历 / 任务 /
        // 视频会议 / 审批)统一走这里,以通讯录侧栏的 greyscale.50 为基准。
        //
        // 为什么要有这个别名:此前通讯录、消息是 greyscale.50,而日历、任务、视频
        // 会议、审批各写 greyscale.000 —— 两边都「有背景色」,可并排切模块时底色会
        // 从浅灰跳到纯白。写成一个 token 后,「二级导航栏」这件事只有一个落点,
        // 下次改也只改这一行(与 railBg 对一级导航栏的做法一致)。
        subNavBg: { value: '{colors.greyscale.50}' },
        ...
        /**
         * 焦点描边色。**随主题翻转**,全站唯一一处焦点色定义。
         *
         * 原值是固定的 `rgb(74, 121, 199)` —— 既不在品牌色阶里,也没有 _dark 档,
         * 于是同一个焦点环在两套主题下长相不同:浅色下比正文蓝弱半档、深色下压在
         * #161616 上发灰,而同一页面里另有 38 处控件自己写 `primary.500`(#3370FF)
         * 当焦点色。「深浅色下控件焦点描边不一致」的根就在这里。
         *
         * 现在两侧都取品牌蓝:浅色 primary.500(#3370FF);深色 primaryDark.600
         * (#88A2E4)—— 与登录页深色态的 `--wm-primary`(#7BAAFB)同档,别用
         * primary.500 原值,深底上中调蓝的可读性不够。
         *
         * 对比度(WCAG 非文本 3:1):#3370FF on #FFFFFF = 3.68:1、on greyscale.50
         * = 3.51:1;#88A2E4 on greyscale.000(#161616) = 8.4:1、on greyscale.100
         * (#242424) = 7.4:1。会中 UI 的恒定深底(primaryDark.50 #0E1626)上浅色
         * 主题仍是 #3370FF = 3.9:1,也过。
         *
         * 用法见 styles/index.css 的「统一焦点描边」块。
         */
        focusRing: {
          value: {
            base: '{colors.primary.500}',
      ...
      shadows: {
         ...
         * `.wm-field:focus-within`:`box-shadow: 0 0 0 3px rgba(37, 99, 235, .12)`。
         *
         * 为什么输入类不用 outline 环:表单行里控件挨得紧(同一行常是
         * Input + Select + Button),带 offset 的环会压到邻居身上;而 box-shadow
         * 不占布局、贴着边框长,与 1px 边框连成一条「粗描边」,正是登录页的观感。
         *
         * 数值:3px 宽度照抄登录页;alpha 比登录页(.12)略高 —— 登录页的边框是
         * 1.5px,这里是全站通用的 1px,光环得补上那半像素的份量。深色下 alpha 必须
         * 更高:半透明蓝压在 #161616 上会被底色吃掉,.12 基本看不见。
         *
         * ⚠️ rgb 分量是 focusRing 两档的字面量展开(#3370FF / #88A2E4)——
         * CSS 没法对 hex 变量取 alpha,color-mix() 的支持线又高于本项目的构建
         * target。改 focusRing 时这两行要跟着改。
         */
        focusRing: {
          value: {
            base: '0 0 0 3px rgba(51, 112, 255, 0.15)',
            _dark: '0 0 0 3px rgba(136, 162, 228, 0.24)',
          },
        },
        ...
        // 而不是四周一圈环形,那是另一个问题,不在本组范围内(表单控件聚焦时的
