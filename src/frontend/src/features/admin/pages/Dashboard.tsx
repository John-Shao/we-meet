...
export const AdminDashboard = () => {
  ...
  const cards: { label: string; value: number; accent?: boolean }[] = [
    { label: t('dashboard.membersActive'), value: data.members.active, accent: true },
  ...
  return (
    <div className={css({ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' })}>
      <h1 className={pageTitle}>{t('dashboard.title')}</h1>
      ...
      >
        <div className={css({ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '0.75rem' })}>
          <h2 className={css({ fontSize: '0.9375rem', fontWeight: 'bold', color: 'greyscale.900' })}>
        ...
        <div className={css({ display: 'flex', justifyContent: 'space-between', marginTop: '0.375rem', fontSize: '0.6875rem', color: 'greyscale.400' })}>
...
const StatCard = ({
  ...
  >
    <span className={css({ fontSize: '0.8125rem', color: 'greyscale.500' })}>{label}</span>
    <span
...
const pageTitle = css({ fontSize: '1.125rem', fontWeight: 'bold', color: 'greyscale.900' })
