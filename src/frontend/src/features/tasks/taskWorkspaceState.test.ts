...
describe('task workspace state', () => {
  ...
  it('uses the open assigned view by default and restores all filters', () => {
    ...
    expect(
      parseTaskWorkspaceState(
        new URLSearchParams(
          'scope=created&status=todo&time=overdue&priority=high&task_list=list-1&view=board&task=42'
        )
      )
    ).toEqual({
      scope: 'created',
  ...
  it('serializes deep links with every workbench filter', () => {
    expect(
      ...
      'scope=all&status=completed&time=all&priority=low&task_list=list%2Fid&view=board&task=task-id'
    )
  })
})
