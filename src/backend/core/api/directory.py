...
def apply_member_list_params(queryset, request):
    """通讯录列表的排序参数(``?ordering=pinyin``)。

    默认仍是 ``user__full_name``(编码序),所有既有调用点(Android 客户端、选人器)
    行为不变。给了 ``ordering=pinyin`` 才换拼音序。

    原来的 ``?from_initial=``(A–Z 索引条的起点跳转)连同 alphabet 端点一起删了 ——
    索引条没活过当晚,这两个能力就没有调用方了;删掉它们比留着「没人调但还在维护」
    的死接口更诚实。
    """
    wants_pinyin = (request.query_params.get("ordering") or "").strip() == "pinyin"
    if not wants_pinyin:
...
class DirectoryMemberSerializer(serializers.Serializer):
    ...
    # A–Z 桶(见 core/services/pinyin.py)。客户端拿它画粘性字母头 ——
...
class DirectoryMemberViewSet(
    ...
    def get_queryset(self):
        return self._member_queryset()

    def _member_queryset(self):
        """成员查询集(组织内、在职、主成员关系,排除设备号)。"""
        organization = get_caller_organization(self.request.user)
        if organization is None:
