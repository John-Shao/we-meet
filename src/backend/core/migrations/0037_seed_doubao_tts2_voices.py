# Seed the full Doubao Seed-TTS 2.0 voice catalog onto the
# ``volcengine/seed-tts-2.0`` model. Idempotent: re-running refreshes each
# voice's label / sort order and adds any missing ones (get/update by the
# ``(model, value)`` unique key). Source: 豆包语音合成模型 2.0 官方音色列表
# (uranus *_bigtts general voices + saturn *_tob role-play / customer-service
# voices). Only the existing ``value`` / ``label`` / ``sort_order`` /
# ``is_active`` fields are used — no schema change.
#
# The placeholder voice ``zh_female_vv_uranus_bigtts`` ("Uranus 女声") seeded
# by 0025 shares its value with "Vivi 2.0" below, so it is updated in place.
from django.db import migrations


TTS_MODEL_CODE = "volcengine/seed-tts-2.0"

# This row pre-exists (created by 0025); reverse restores its original state.
PLACEHOLDER_VALUE = "zh_female_vv_uranus_bigtts"

# (voice_type, display label) in catalog order. sort_order = index * 10.
VOICES = [
    ...
    # 角色扮演 / 客服 (saturn *_tob, 指令遵循 + COT/QA)
    ...
    ("saturn_zh_female_aojiaonvyou_tob", "傲娇女友 2.0"),
    ("saturn_zh_female_bingjiaojiejie_tob", "病娇姐姐 2.0"),
    ("saturn_zh_female_chengshujiejie_tob", "成熟姐姐 2.0"),
    ("saturn_zh_female_keainvsheng_tob", "可爱女生 2.0"),
    ("saturn_zh_female_nuanxinxuejie_tob", "暖心学姐 2.0"),
    ("saturn_zh_female_tiexinnvyou_tob", "贴心女友 2.0"),
    ("saturn_zh_female_wenrouwenya_tob", "温柔文雅 2.0"),
    ("saturn_zh_female_wumeiyujie_tob", "妩媚御姐 2.0"),
    ("saturn_zh_female_xingganyujie_tob", "性感御姐 2.0"),
    ("saturn_zh_male_aiqilingren_tob", "傲气凌人 2.0"),
    ("saturn_zh_male_aojiaogongzi_tob", "傲娇公子 2.0"),
    ("saturn_zh_male_aojiaojingying_tob", "傲娇精英 2.0"),
    ("saturn_zh_male_aomanshaoye_tob", "傲慢少爷 2.0"),
    ("saturn_zh_male_badaoshaoye_tob", "霸道少爷 2.0"),
    ("saturn_zh_male_bingjiaobailian_tob", "病娇白莲 2.0"),
    ("saturn_zh_male_bujiqingnian_tob", "不羁青年 2.0"),
    ("saturn_zh_male_chengshuzongcai_tob", "成熟总裁 2.0"),
    ("saturn_zh_male_cixingnansang_tob", "磁性男嗓 2.0"),
    ("saturn_zh_male_cujingnanyou_tob", "醋精男友 2.0"),
    ("saturn_zh_male_fengfashaonian_tob", "风发少年 2.0"),
    ("saturn_zh_male_fuheigongzi_tob", "腹黑公子 2.0"),
...
def seed_voices(apps, schema_editor):
    ...
    try:
        model = AIModel.objects.get(code=TTS_MODEL_CODE)
...
def unseed_voices(apps, schema_editor):
    ...
    # Remove the voices this migration added, but keep the placeholder row
    # 0025 owns and restore its original label / sort order.
    AIVoice.objects.filter(
        model=model, value__in=[value for value, _ in VOICES]
    ).exclude(value=PLACEHOLDER_VALUE).delete()
    AIVoice.objects.filter(model=model, value=PLACEHOLDER_VALUE).update(
        label="Uranus 女声", sort_order=10
    )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("core", "0036_aiagentprofile_agent_type"),
    ]
