...
# Switch apk to Aliyun mirror + point pip at Aliyun PyPI (for `pip install --upgrade pip`
# step) to avoid frequent `Connection reset by peer` errors.
#
# IMPORTANT: do NOT redirect uv. src/backend/uv.lock pins every package's source to
# pypi.org and `uv sync --locked` strictly verifies. Redirecting uv changes resolved
# package sources to the mirror, triggering "lockfile needs to be updated" failures.
# uv has resilient retries; UV_HTTP_TIMEOUT=300 covers slow PyPI access from CN.
